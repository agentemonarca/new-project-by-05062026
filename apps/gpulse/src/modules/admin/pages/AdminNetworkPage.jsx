...
export function AdminNetworkPage() {
  const { state, networkActions } = useAdmin();
  ...
  const u = useMemo(() => (state.users || []).find((x) => x.id === userId.trim()), [state.users, userId]);
  ...
  return (
    <motion.div className="space-y-6" variants={staggerContainer} initial="hidden" animate="show">
      <AdminPageHeader
        ...
        subtitle="Volumen agregado por pierna. Acciones simuladas hasta API de genealogía."
      />
      ...
      <motion.div variants={fadeUpBlur} className="rounded-2xl border border-dashed border-white/15 bg-slate-950/40 p-4 text-center text-sm text-slate-500">
        Visualización de árbol (placeholder) — integra react-flow o canvas en fase 2.
      </motion.div>
      ...
      <motion.div variants={fadeUpBlur} className="rounded-2xl border border-white/[0.08] bg-slate-950/60 p-5">
        ...
        <input
          ...
          placeholder="ID usuario objetivo (ej. U-2041)"
        ...
        {u ? (
          ...
          <p className="mt-2 text-xs text-rose-400/90">Usuario no encontrado.</p>
        ) : null}
