...
/**
 * Contenedor lateral reutilizable (overlay + aside animado) para flujos admin.
 * @param {{
 *   open: boolean,
 *   onClose: () => void,
 *   eyebrow?: string,
...
function AdminSidePanelInner({
  ...
  subtitle,
  children,
  widthClassName = 'md:max-w-xl lg:max-w-2xl',
  asideClassName = '',
}) {
  return (
    <AnimatePresence>
      {open ? (
        <>
          <motion.button
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[120] bg-slate-950/70 backdrop-blur-sm md:bg-slate-950/45"
          ...
          <motion.aside
            ...
            className={`fixed inset-y-0 right-0 z-[121] flex w-full max-w-[100vw] flex-col border-l border-cyan-500/15 bg-[#070a12]/98 shadow-[0_0_60px_rgba(0,0,0,0.55)] backdrop-blur-2xl ${widthClassName} ${asideClassName}`}
          >
            ...
            <div className="custom-scrollbar min-h-0 flex-1 overflow-y-auto px-4 py-4 lg:px-5 lg:py-5">
              {children}
            </div>
          </motion.aside>
        </>
