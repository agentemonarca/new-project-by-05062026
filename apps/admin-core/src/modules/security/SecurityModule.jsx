...
function SecurityModuleInner() {
  const { projectLogs, currentProject, projectSecurity, securityActions } = useAdminCore();
    ...
    () => [
      {
        ...
        render: (r) => {
          const row = displayAuditRow(r);
          return formatTs(row.timestamp || r.label || r.ts);
        },
      },
      {
        key: 'action',
        ...
        render: (r) => {
          ...
          return (
            <span className="inline-flex flex-wrap items-center gap-1.5">
              {row.action || '—'}
              {critical ? (
                <span className="rounded bg-rose-500/20 px-1 py-0.5 text-[9px] font-bold uppercase text-rose-100">
                  crítico
                </span>
              ) : null}
            </span>
      ...
      {
        key: 'targetId',
        header: 'Usuario / target',
        className: 'font-mono text-xs',
        render: (r) => displayAuditRow(r).targetId || '—',
      },
      {
        key: 'txId',
        header: 'tx / orden',
        ...
        render: (r) => {
          const { meta } = displayAuditRow(r);
          return meta?.txId || meta?.orderId || r.txId || r.orderId || '—';
        },
      },
      {
        key: 'admin',
        header: 'Admin',
        render: (r) => displayAuditRow(r).admin || '—',
      },
      {
        key: 'risk',
        header: 'Riesgo',
        render: (r) => {
          const row = displayAuditRow(r);
          const risk = row.meta?.risk ?? r.risk;
          return <span className="rounded-md bg-white/5 px-1.5 py-0.5 text-xs">{risk ?? '—'}</span>;
        },
      },
    ],
    [],
    ...
    <motion.div className="space-y-6" variants={staggerContainer} initial="hidden" animate="show">
      <motion.div variants={fadeUpBlur}>
        <AdminPageHeader
          ...
          subtitle="Logs operativos y contención por proyecto"
        />
      </motion.div>
      ...
      <motion.div variants={fadeUpBlur} className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-white/[0.08] bg-slate-950/60 p-4">
          ...
          <button
            type="button"
            onClick={() => open('Bloquear IP', `¿Bloquear ${ip}?`, () => securityActions.blockIp(ip.trim()))}
            className="mt-2 w-full rounded-lg bg-rose-600/80 py-2 text-xs font-semibold text-white"
          >
            Bloquear
          </button>
          <p className="mt-2 text-[10px] text-slate-500">
            IPs activas: {projectSecurity.blockedIps?.join(', ') || '—'}
          </p>
        </div>
        <div className="rounded-2xl border border-white/[0.08] bg-slate-950/60 p-4">
      ...
      <motion.div variants={fadeUpBlur}>
        <h3 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">
          Security log · {currentProject || '—'}
        </h3>
        <ul className="space-y-2">
          {(projectSecurity.securityLogs || []).map((row) => (
            <li
              key={row.id}
      ...
      <motion.div variants={fadeUpBlur}>
        <h3 className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500">Auditoría global (filtrada)</h3>
        <AdminTable columns={columns} rows={projectLogs} rowKey={(r) => r.id} emptyLabel="Sin eventos registrados." />
      </motion.div>
...
export const SecurityModule = memo(SecurityModuleInner);
